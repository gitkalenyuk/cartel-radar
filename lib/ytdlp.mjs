...
/**
 * Додаткові місця, де yt-dlp опиняється в зібраному застосунку.
 * Електрон кладе вміст extraResources у <resources>/, тобто на рівень ВИЩЕ за
 * саму програму (<resources>/app). Саме через це в інсталяторі yt-dlp не
 * знаходився, хоч і лежав поруч — і радар мовчки не мав даних.
 */
export function extraBinDirs() {
  ...
  try { dirs.push(path.join(path.dirname(process.execPath), "bin")); } catch {}
  return dirs;
}

/** Усі місця, де ми шукали yt-dlp — щоб у разі невдачі показати це користувачу. */
export function searchedBinPaths() {
  const names = process.platform === "win32" ? ["yt-dlp.exe"] : ["yt-dlp", "yt-dlp_macos"];
  const roots = [
    localBinDir(),
    ...BIN_DIRS.map((d) => path.join(appRoot(), d)),
    ...extraBinDirs(),
  ...
  return roots.map((d) => path.join(d, names[0]));
}
...
export function ytdlpPath({ refresh = false } = {}) {
  ...
  // усі шляхи вже абсолютні
  for (const dir of [localBinDir(), ...extraBinDirs(), ...BIN_DIRS.map((d) => path.join(appRoot(), d))]) {
...
// ── самостійне завантаження yt-dlp ────────────────────────────────────────
/**
 * Якщо yt-dlp не знайдено (нова система, невдале завантаження під час
 * встановлення, антивірус видалив файл) — застосунок має вміти підтягнути
 * його сам, а не залишати користувача без даних. Кладемо в теку даних:
 * вона доступна на запис завжди, на відміну від теки застосунку.
 */
export function localBinDir() {
  return path.join(DATA_DIR, "bin");
}
...
export async function installYtdlp({ onProgress } = {}) {
  ...
  if (!res.ok) throw new Error("Не вдалося завантажити yt-dlp: сервер відповів " + res.status);
  ...
  // у відповіді має бути справжній бінарник, а не HTML-сторінка помилки
  if (buf.length < 100000) throw new Error("Завантажений файл замалий — схоже, це не yt-dlp");
  ...
  cachedVersion = null;
  const found = ytdlpPath({ refresh: true });
  if (!found) throw new Error("yt-dlp завантажено, але знайти його не вдалося: " + target);
  return { path: found, version: await ytdlpVersion(), bytes: buf.length };
}
