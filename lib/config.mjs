...
// Дані живуть у постійній теці користувача, а НЕ поруч із кодом:
// інакше оновлення застосунку стирало б ключі та ніші.
export const DATA_DIR = process.env.CARTEL_RADAR_DATA || process.env.NICHE_RADAR_DATA
  || path.join(os.homedir(), "Library", "Application Support", "CartelRadar", "data");
